...
impl serialize::Encoder<Error> for Encoder {
    ...
    fn emit_tuple<F>(&mut self, len: uint, f: F) -> Result<(), Error> where
        F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        self.emit_seq(len, f)
    }
    fn emit_tuple_arg<F>(&mut self, idx: uint, f: F) -> Result<(), Error> where
        F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
    ...
    fn emit_seq<F>(&mut self, _len: uint, f: F) -> Result<(), Error> where
        F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
    ...
    fn emit_seq_elt<F>(&mut self, _idx: uint, f: F) -> Result<(), Error> where
        F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        f(self)
    }
    fn emit_map<F>(&mut self, len: uint, f: F) -> Result<(), Error> where
        F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
    ...
    fn emit_map_elt_key<F>(&mut self, _idx: uint, mut f: F)
                           -> Result<(), Error> where
        F: FnMut(&mut Encoder) -> Result<(), Error>
    {
...
impl serialize::Decoder<DecodeError> for Decoder {
    ...
    fn read_option<T, F>(&mut self, f: F) -> Result<T, DecodeError> where
        F: FnOnce(&mut Decoder, bool) -> Result<T, DecodeError>
    {
        match self.toml {
            Some(..) => f(self, true),
            None => f(self, false),
