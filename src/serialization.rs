...
impl serialize::Encoder<Error> for Encoder {
    ...
    fn emit_struct<F>(&mut self, _name: &str, _len: uint, f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        match mem::replace(&mut self.state, Start) {
            NextKey(key) => {
    ...
    fn emit_struct_field<F>(&mut self, f_name: &str, _f_idx: uint, f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
    ...
    fn emit_tuple<F>(&mut self, len: uint, f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        self.emit_seq(len, f)
    }
    fn emit_tuple_arg<F>(&mut self, idx: uint, f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        self.emit_seq_elt(idx, f)
    }
    fn emit_tuple_struct<F>(&mut self, _name: &str, _len: uint, _f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        unimplemented!()
    }
    fn emit_tuple_struct_arg<F>(&mut self, _f_idx: uint, _f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        unimplemented!()
    }
    ...
    fn emit_seq<F>(&mut self, _len: uint, f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
    ...
    fn emit_seq_elt<F>(&mut self, _idx: uint, f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        f(self)
    }
    fn emit_map<F>(&mut self, len: uint, f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        self.emit_struct("foo", len, f)
    }
    fn emit_map_elt_key<F>(&mut self, _idx: uint, f: F) -> Result<(), Error>
        where F: FnMut(&mut Encoder) -> Result<(), Error>
    {
        ...
        // TODO: Find out how to do this without renaming f or changing the fn signature.
        let mut g = f;
        try!(g(self));
        match self.state {
            NextKey(_) => Ok(()),
    ...
    fn emit_map_elt_val<F>(&mut self, _idx: uint, f: F) -> Result<(), Error>
        where F: FnOnce(&mut Encoder) -> Result<(), Error>
    {
        f(self)
    }
...
impl serialize::Decoder<DecodeError> for Decoder {
    ...
    {
        // TODO: Find out how to do this without calling f for each i.
        unimplemented!()
...
//        Err(first_error.unwrap_or_else(|| self.err(NoEnumVariants)))
    ...
    fn read_enum_variant_arg<T, F>(&mut self, _a_idx: uint, f: F) -> Result<T, DecodeError>
        where F: FnOnce(&mut Decoder) -> Result<T, DecodeError>
    {
        f(self)
    }
    ...
    fn read_enum_struct_variant_field<T, F>(&mut self, _f_name: &str, _f_idx: uint, _f: F)
    ...
    fn read_tuple<T, F>(&mut self, tuple_len: uint, f: F)
        ...
        where F: FnOnce(&mut Decoder) -> Result<T, DecodeError>
    {
        self.read_seq(move|d: &mut Decoder, len: uint| {
    ...
    fn read_option<T, F>(&mut self, f: F) -> Result<T, DecodeError>
                where F: FnOnce(&mut Decoder, bool) -> Result<T, DecodeError>
    {
        match self.toml {
            Some(..) => f(self, true),
            None => f(self, false),
    ...
    fn read_seq_elt<T, F>(&mut self, idx: uint,
                       f: F)
        -> Result<T, DecodeError>
        where F: FnOnce(&mut Decoder) -> Result<T, DecodeError>
    {
    ...
    fn read_map_elt_key<T, F>(&mut self, idx: uint, f: F) -> Result<T, DecodeError>
        where F: FnOnce(&mut Decoder) -> Result<T, DecodeError>
    {
        match self.toml {
    ...
    fn read_map_elt_val<T, F>(&mut self, idx: uint, f: F) -> Result<T, DecodeError>
        where F: FnOnce(&mut Decoder) -> Result<T, DecodeError>
    {
        match self.toml {
            Some(Table(ref table)) => {
